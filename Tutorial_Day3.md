...
> `index` index that will appear "AUTOMATICALLY" in front of all output files generated by this command (in this example your files will be named out_index.1.bt2, out_index.2.bt2 and so on, in total you should get 6 index files). YOU DO NOT NEED TO EDIT OR RENAME THESE FILES: THESE ARE AUTOMATIC OUTPUT FILES.
...
for i in *_R1_clean.fastq.gz; do
  base="${i%_R1_clean.fastq.gz}"; bowtie2 --very-fast -x /PATH/TO/index/contigs.anvio.fa.index -1 $i -2 "$base"_R2_clean.fastq.gz -S /PATH/TO/4_mapping/"${base}".sam 
done
```
</details>
